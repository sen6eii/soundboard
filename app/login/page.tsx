...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (result) {
      ...
      setError("No account found with that email. Did you mean to register?")
    }
  }
  ...
  return (
    <div className="min-h-screen bg-ink flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm">
        <h1 className="font-display font-bold text-text text-2xl mb-1">Welcome back</h1>
        <p className="font-body text-[13px] text-dim mb-8">Sign in to your workspace</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {error && (
            <p className="font-body text-[12px] text-red">{error}</p>
          )}
